...
def prepare_multi_scale_data(src_path, dst_path, gap=200, subsize=1024, scales=[0.5, 1.0, 1.5], num_process=32):
    ...
    #split_test = splitbase_trainval(osp.join(src_path, 'val'), dst_test_base_path,
    #                            gap=gap, subsize=subsize, num_process=num_process)
    #for scale in scales:
    ...
    #               wordname_15)
    DOTA2COCOTrain(dst_test_base_path,
...
if __name__ == '__main__':
    # single scale
    prepare_multi_scale_data('data/dota',
                             'data/dota_1024_train_val', scales=[1.0], gap=824)
    print('done')
