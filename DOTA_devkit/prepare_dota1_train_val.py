...
def prepare_multi_scale_data(src_path, dst_path, gap=200, subsize=1024, scales=[0.5, 1.0, 1.5], num_process=32):
    ...
    generate_json_labels(dst_val_path, osp.join(
        dst_val_path, 'val.json'))
    ...
    #                wordname_15)
    DOTA2COCOTrain(dst_val_path,
                   osp.join(dst_val_path, 'val.json'),
                   wordname_15)
...
if __name__ == '__main__':
    # single scale
    prepare_multi_scale_data('/home/angtordom1/OrientedRepPoints/data/dota',
                             '/home/angtordom1/OrientedRepPoints/data/dota_1024', scales=[1.0])
    print('done')
