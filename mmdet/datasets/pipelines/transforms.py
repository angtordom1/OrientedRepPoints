...
class ColorJitter(object):
    ...
    def __init__(self, brightness_limit=[0.1, 0.3], contrast_limit=[0.1, 0.3], p=0.2):
        self.image_aug = albumentations.RandomBrightnessContrast(
    ...
    def __call__(self, results):
        img = results['img']
        aug_img = self.image_aug(image=img)['image']
        results['img'] = aug_img.astype(np.uint8)
        return results
...
class HSVAugment(object):
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += ('(hgain={}, sgain={}, vgain={})').format(self.hgain,
                                                              self.sgain,
                                                              self.vgain)
